use axum::{
    extract::{State, Extension}, // Added Extension
    http::StatusCode,
    response::{IntoResponse, Json},
};
// Removed unused: async_trait, middleware::Next, http::Request, Response, axum_extra::extract::cookie::Cookie
...
use crate::{
    ...
    errors::AppError,
    models::{AppState, LoginRequest, NewUser, UpdateUserProfilePayload}, // Removed unused User model import here
};
...
pub async fn login_handler(
    ...
    let user = state.user_service.authenticate_user(&payload.email, &payload.password).await?;
